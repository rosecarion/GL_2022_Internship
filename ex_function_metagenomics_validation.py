...
"""
from overlapping_functions_amplicon_metagenomics_validation import *
...
def check_raw_multiqc_outputs(file : path, file_prefixes_in_multiqc : path):
...
def check_filtered_multiqc_outputs(file : path, file_prefixes_in_multiqc : path):   
...
file_prefixes_in_raw_multiqc = df["Sample"].tolist()

# for filtered multiqc output check
zip_file = zipfile.ZipFile(fastqc_dir + str(additional_prefix) + "filtered_multiqc_data.zip")
df = pd.read_csv(zip_file.open("multiqc_general_stats.txt"), sep = "\t", usecols = ["Sample"])
file_prefixes_in_filtered_multiqc = df["Sample"].tolist()
            ...
            if single_ended == False:
                for R_suffix in rawR_suffixes:
                    with vp.payload(
                        payloads=[
                            {
                                "sample" : sample + R_suffix, "file_prefixes_in_multiqc" : file_prefixes_in_raw_multiqc
                            }
                        ]
                    ):
                ...
                with vp.payload(
                    payloads=[
                        {
                            "sample" : sample + suffix, "file_prefixes_in_multiqc" : file_prefixes_in_raw_multiqc
                        }
                    ]
                ):
                    vp.add(check_raw_multiqc_outputs)
            ...
            if single_ended == False:
                for filtered_R_suffix in filteredR_suffixes:
                        with vp.payload(
                            payloads=[
                            {
                                "sample" : sample + filtered_R_suffix, "file_prefixes_in_multiqc" : file_prefixes_in_filtered_multiqc
                            }
                        ]
                    ):
                ...
                with vp.payload(
                    payloads=[
                        {
                            "sample" : sample + suffix, "file_prefixes_in_multiqc" : file_prefixes_in_filtered_multiqc
                        }
                    ]
                ):
                    vp.add(check_filtered_multiqc_outputs)
